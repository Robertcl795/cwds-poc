...
test('renders navigation verification route and validates keyboard + dialog focus flows', async ({ page }) => {
  await page.goto('/#verification-navigation');

  await expect(page.getByRole('heading', { name: 'Navigation Verification Demo' })).toBeVisible();
  ...
  await expect(page.locator('#verification-navigation-list-log')).toContainText('Action: open-report');
});
