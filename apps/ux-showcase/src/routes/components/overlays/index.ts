import { createContextMenu, createPrimitiveButton, createPrimitiveIconButton, createPrimitiveTooltip } from '@ds/components';
...
export function renderOverlayComponentsShowcase(container: HTMLElement): void {
  const page = document.createElement('main');
  page.className = 'overlays-page';

  const heading = document.createElement('h1');
  heading.textContent = 'Overlay Components Demo';

  const intro = document.createElement('p');
  intro.textContent =
    'Overlay surfaces plus web-component adapters: native-first tooltip and context menu alongside combobox and advanced select baselines.';

  const grid = document.createElement('section');
  grid.className = 'overlays-grid';

  const tooltipCard = document.createElement('article');
  tooltipCard.className = 'overlays-card';
  ...
  contextCard.className = 'overlays-card';
  ...
  contextTarget.dataset.overlaysContextTarget = 'true';

  const contextLog = document.createElement('p');
  contextLog.className = 'overlays-log';
  ...
  formCard.className = 'overlays-card';
  formCard.dataset.overlaysAdapterAudit = 'true';

  const formTitle = document.createElement('h2');
  formTitle.textContent = 'Combobox + Advanced Select';

  const form = document.createElement('form');
  form.className = 'overlays-form';
  ...
  assignee.dataset.overlaysAdapter = 'combobox';
  assignee.options = [
  ...
  environment.dataset.overlaysAdapter = 'advanced-select';
  environment.options = [
  ...
  result.className = 'overlays-result';
